...
#' @section Aesthetics:
#' `geom_cfb_headshots()` understands the following aesthetics (required aesthetics are in bold):
#' \itemize{
#'   \item{**x**}{ - The x-coordinate.}
#'   \item{**y**}{ - The y-coordinate.}
#'   \item{**player_id**}{ - The players' ESPN id.}
...
#'   geom_cfb_headshots(aes(player_id = player_id), height = 0.2) +
...
#' ggplot(df, aes(x = a, y = b)) +
#'   geom_cfb_headshots(aes(player_id = player_id), height = 0.2, alpha = 0.5) +
#'   geom_label(aes(label = player_name), nudge_y = -0.35, alpha = 0.5) +
#'   coord_cartesian(xlim = c(0.75, 3.25), ylim = c(0.7, 3.25)) +
#'   theme_void()
#'
...
#' ggplot(df, aes(x = a, y = b)) +
#'   geom_cfb_headshots(aes(player_id = player_id, colour = colour), height = 0.2) +
#'   geom_label(aes(label = player_name), nudge_y = -0.35, alpha = 0.5) +
#'   coord_cartesian(xlim = c(0.75, 3.25), ylim = c(0.7, 3.25)) +
...
geom_cfb_headshots <- function(mapping = NULL, data = NULL,
...
GeomCFBheads <- ggplot2::ggproto(
  ...
  draw_panel = function(data, panel_params, coord, na.rm = FALSE) {
    ...
    grobs <- lapply(seq_along(data$player_id), build_grobs, alpha = data$alpha, colour = data$colour, data = data, teams = FALSE)
